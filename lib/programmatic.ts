...
const DESCRIPTION_TEMPLATES: readonly Tpl[] = [
  (c, s) =>
    `${s.label} in ${c.name}? Sarte Global bouwt een betaalbare, professionele en goed vindbare ${s.noun} voor ondernemers in ${c.name} en ${c.region} — al vanaf €995. Sterke SEO en focus op conversie. Vraag vrijblijvend een offerte aan.`,
  (c, s) =>
    `Een ${s.noun} laten maken in ${c.name}? Wij ontwerpen betaalbare, responsive websites die ondernemers in ${c.province} meer klanten opleveren. Transparante prijzen vanaf €995, snelle oplevering.`,
  (c, s) =>
...
const BENEFIT_POOL: readonly Tpl[] = [
  ...
  () => `Betaalbaar vanaf €995 — transparante prijs, geen verrassingen`,
];
